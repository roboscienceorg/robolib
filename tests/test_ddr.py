...
def test_DDR_circle_IK_and_back():
    ...
    t = np.linspace(0, 40*np.pi, 5001)
    t = t[1:]
    t_steps = np.linspace(40.0*np.pi/5000, 40.0*np.pi/5000, 5000)
    ...
    assert np.allclose(x, new_x, atol=0.1) == True
    assert np.allclose(y, new_y, atol=0.1) == True
